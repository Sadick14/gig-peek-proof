...
const OpenDeal = () => {
  ...
  const handleSubmitProof = async () => {
    if (!proofPreview.trim() || !foundDeal) return;
  ...
  return (
    <div className="max-w-2xl space-y-8">
      <div>
        <h1 className="text-3xl font-bold mb-2">Open a Deal</h1>
        <p className="text-muted-foreground">
          Enter a Deal ID provided by your client to view details and submit work
        </p>
      </div>
      ...
      <Card className="p-6 bg-gradient-card border-border/50 backdrop-blur-sm shadow-card">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-xl font-semibold">Find Your Gig</h2>
        </div>
          ...
          {foundDeal.status === 'waiting_proof' && (
            <div className="space-y-4">
              ...
              <Button
                ...
                disabled={!proofPreview.trim() || isSubmitting}
